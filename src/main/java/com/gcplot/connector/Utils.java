...
public abstract class Utils {
    ...
    public static String toBase64(String s) {
        try {
            return BaseEncoding.base64().omitPadding().encode(s.getBytes("UTF-8"));
    ...
    public static String fromBase64(String s) {
        try {
            return new String(BaseEncoding.base64().omitPadding().decode(s), "UTF-8");
        } catch (UnsupportedEncodingException e) {
            throw Exceptions.runtime(e);
        }
    }
    ...
    public static Properties fromString(String sourceConfig) {
        ...
        if (sourceConfig != null) {
            for (String s : Splitter.on(";").split(sourceConfig)) {
                int index = s.indexOf('=');
                if (index != -1) {
